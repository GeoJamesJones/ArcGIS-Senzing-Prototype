...
class MyBuffer(threading.local):
  def __init__(self):
    ...
    #arcpy.AddMessage("Created new Buffer {} of type {}".format(self.buf,type(self.buf)))
...
def resize_return_buffer3(buf_, size_):
  ...
  try:
    if not tls_var3.buf:
      ...
      #arcpy.AddMessage("RESIZE_RETURN_BUF {}:{}/{}".format(buf_,size_,tls_var3.bufSize))
      foo = tls_var3.buf
      tls_var3.buf = create_string_buffer(size_)
      tls_var3.bufSize = size_
      ...
      #arcpy.AddMessage("AttributeError RESIZE_RETURN_BUF {}:{}".format(buf_,size_))
      tls_var3.buf = create_string_buffer(size_)
      #arcpy.AddMessage("Created new Buffer {}".format(tls_var3.buf))
      tls_var3.bufSize = size_
...
class G2Engine(object):
    ...
    def initV2(self, engine_name_, ini_params_, debug_=False):
        ...
        self._debug = debug_
        if self._debug:
            arcpy.AddMessage("Initializing G2 engine")
